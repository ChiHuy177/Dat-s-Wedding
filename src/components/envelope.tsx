...
/**
 * Screen one: a full-viewport overlay showing nothing but the card. The rest of
 * the site stays behind it, locked, until `onOpened` fires.
 */
export function Envelope({ onOpened }: { onOpened: () => void }) {
  ...
  function handleOpen() {
    if (open) return;
    setOpen(true);
    // Let the flap swing and the card slide out before handing over to the page.
    window.setTimeout(onOpened, 1250);
  }

  return (
    <motion.section
      className="fixed inset-0 z-50 overflow-y-auto bg-[radial-gradient(120%_90%_at_50%_0%,#3E4A22_0%,#2B2A26_60%,#1c1b17_100%)] text-[#f8ece2]"
      exit={{ opacity: 0, scale: 1.05 }}
      transition={{ duration: 0.65, ease: EASE_SMOOTH }}
    >
      <div className="relative flex min-h-full flex-col items-center justify-center overflow-hidden px-8 py-14 text-center">
